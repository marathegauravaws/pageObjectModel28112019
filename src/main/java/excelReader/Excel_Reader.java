...
public class Excel_Reader {
	...
	@SuppressWarnings("deprecation")
	public String getCustomerData(String sheetName, String dataRequired, int col_Num) {
		String data = "";
		String datareturned = "";
		int NumericData;
		try {
			...
			for (int i = 0; i <= row; i++) {
				if (sheet.getRow(i).getCell(0).getStringCellValue().equalsIgnoreCase(dataRequired)) {
					XSSFCell cell = sheet.getRow(i).getCell(col_Num);
					if (cell.getCellType() == Cell.CELL_TYPE_STRING) {
						datareturned = sheet.getRow(i).getCell(col_Num).getStringCellValue().toString();

					} else {
						NumericData = (int) sheet.getRow(i).getCell(col_Num).getNumericCellValue();
						datareturned=String.valueOf(NumericData);						
					}
					break;
				}
			}
